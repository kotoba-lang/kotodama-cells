# yorishiro_argparse-demo

Pregel cell for the **argparse-demo** yorishiro (kami: `bin:argparse-demo`,
...
The `argparse-demo` binary must be available on the
cell runtime's PATH (or supplied as an absolute path in the kami
manifest at `00-contracts/kami/argparse-demo.kami.json`). The cell
verifies this via `shutil.which()` at the start of every invocation
and returns `error: "binary not found on PATH"` if missing.

## Lexicon SSoT

`00-contracts/lexicons/ai/etzhayyim/yorishiro/argparse-demo/<op>.json`

## MCP exposure

`20-actors/kotoba-kotodama/mcp/yorishiro-argparse-demo-mcp/` (stdio + Streamable HTTP)

## Regenerate

```bash
yorishiro regen argparse-demo
```
...
```json
{
  "mcpServers": {
    "etzhayyim-yorishiro-argparse-demo": {
      "command": "node",
      "args": ["/ABSOLUTE/PATH/TO/repo/20-actors/kotoba-kotodama/mcp/yorishiro-argparse-demo-mcp/src/cli.ts"]
    }
  }
}
```
...
local-binary case shown above, `argparse-demo` must be
on the host's PATH.
